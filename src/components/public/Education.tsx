                ...
                <div className={`relative flex flex-col gap-4 sm:flex-row sm:items-center ${i % 2 ? 'sm:flex-row-reverse' : ''}`}>
                  ...
                  <div className={`rounded-2xl border border-white/10 bg-white/5 p-6 transition hover:border-indigo-400/40 hover:bg-white/10 sm:w-1/2 ${i % 2 ? 'sm:mr-8' : 'sm:ml-8'}`}>
                    <div className="flex items-start gap-4">
                      {e.logo_url ? (
                        <img src={e.logo_url} alt={e.institution} className={`h-12 w-auto max-w-24 shrink-0 rounded-lg object-contain ${e.institution.toLowerCase().includes('utkal') ? 'bg-white/90 p-1' : ''}`} onError={(ev) => { ev.currentTarget.style.display = 'none'; ev.currentTarget.nextElementSibling?.classList.remove('hidden'); }} />
                      ) : null}
