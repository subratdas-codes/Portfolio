            ...
            <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-8">
              ...
              <div className="flex gap-3 pt-2">
                {socials.map((s) => { const Icon = ICONS[s.icon] ?? Github; return (
                  <a key={s.id} href={s.url} target="_blank" rel="noreferrer" className="grid h-11 w-11 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-300 transition hover:scale-110 hover:border-indigo-400/50 hover:text-indigo-300"><Icon size={18} /></a>
                ); })}
              </div>
            </div>
          </Reveal>
