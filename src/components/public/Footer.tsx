        ...
        <div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
          ...
          <div className="flex gap-3">
            {socials.map((s) => { const Icon = ICONS[s.icon] ?? Github; return (
              <a key={s.id} href={s.url} target="_blank" rel="noreferrer" className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-300 transition hover:scale-110 hover:text-indigo-300"><Icon size={18} /></a>
            ); })}
          </div>
        </div>
